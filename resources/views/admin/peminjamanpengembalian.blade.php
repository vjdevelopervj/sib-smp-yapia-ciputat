            ...
            <main class="p-6 animate__animated animate__fadeIn animate__faster">
                ...
                <div id="peminjamanSection" class="animate__animated">
                    ...
                    <div
                        ...
                        <form id="peminjamanForm" action="{{ route('admin.peminjaman.store') }}" method="POST"
                            class="space-y-4">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="tanggal_pinjam"
                                        class="block text-sm font-medium text-gray-300 bg-[#131924]">Tanggal
                                        Pinjam</label>
                            ...
                            <div class="hidden sm:flex sm:items-center sm:justify-between w-full">
                                ...
                                <div class="overflow-x-auto max-w-[300px]">
                                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px flex-nowrap"
                                        ...
                                        @if ($borrowings->hasMorePages())
                                            <a href="{{ $borrowings->nextPageUrl() }}"
                                                class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-5 bg-[#1e293b] text-sm font-medium text-gray-300 hover:bg-gray-700 transition-colors">
            ...
            returnButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    confirmReturnBtn.onclick = function() {
                        const tanggalDikembalikan = document.getElementById(
                            'modalTanggalKembali').value;
                        const kondisiKembali = document.getElementById('modalKondisiKembali')
                            .value;
                        const catatan = document.getElementById('modalCatatanKembali').value;

                        fetch(`/admin/peminjaman/${itemId}/return`, {
                                method: 'POST',
                                headers: {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    showNotification('success',
                                        'Pengembalian barang berhasil diproses!');
                                    // Refresh the page after 1 second to show updated data
                                    setTimeout(() => {
                                        window.location.reload();
                                    }, 1000);
                                } else {
                                    showNotification('error', 'Terjadi kesalahan: ' + data
                                        .message);
                                }
                            })
                            .catch(error => {
